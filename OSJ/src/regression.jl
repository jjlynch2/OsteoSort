...
function REGSL(m1, m2, RL, RR)
	...
	for o in 1:size(m1,1)
		for x in 1:size(m2,1)
			...
			Results[ri,1] = o #index of left
			Results[ri,2] = x #index of right
			Results[ri,3] = pVal #p-value
			Results[ri,4] = n #reference sample size
			Results[ri,5] = r2 #r-square
			for j in m_counter_1_o
				Results[ri,j+5] = 1
			end
			for j in m_counter_2_o
				Results[ri,j+5+size(m1[o,1:end],1)] = 1
			end
			ri += 1
		end
	end

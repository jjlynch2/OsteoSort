...
output_function <- function(hera1, method = "exclusion", type = "csv") {
	...
	if(method == "exclusion") {
		...
		if(type == "plot3") {
			jpeg(paste("graph",hera1[[1]],"-", hera1[[2]],".jpg",sep=''),height = 400, width = 400)
			dev.control('enable')
			plot(hera1[[4]],hera1[[3]], xlab = "Measurement", ylab = "Stature")
			points(hera1[[6]],hera1[[5]],col="blue",pch=16, cex=1.5)
			points(hera1[[6]],hera1[[7]],col="red",pch=16, cex=1.5)
			#df1 <- data.frame(hera1[[6]], hera1[[5]])
			#df2 <- data.frame(hera1[[6]], hera1[[7]])
			#matlines(df1, df2, col=c("red"), lty=2)
			matlines(hera1[[4]],hera1[[8]][,1],  col=c("red"))
			matlines(hera1[[4]],hera1[[8]][,2], col=c("blue"), lty = 4)
			matlines(hera1[[4]],hera1[[8]][,3], col=c("blue"), lty = 4)
			dev.off()
		}
	}
	if(method == "2D") {

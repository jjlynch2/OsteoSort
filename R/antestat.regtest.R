...
print(X)
		pm1 <- predict(lm1, newdata = data.frame(measurement = X[6]), interval = "prediction", level = prediction_interval)

		tt <- abs(round(pm1[1], digits=2) - X[2]) / ( summary.lm((lm1))$sigma * sqrt( 1+(1/nref) + ((X[6] - mean(measurement))^2) / (nref * sd(measurement)^2) ) )
		...
		if(!alphatest) {
			if(X[2] <= pm1[3] && X[2] >= pm1[2]) { #checks if predicted falls within prediction interval for the predictors
				within <- "Cannot Exclude"
			}
			else within <- "Excluded"
		}
		...
		if(output_options[2]) {
			...
			points(pm1[1],X[6],col="blue",pch=16, cex=1.5)
		...
		data.frame(X[1],X[2],X[3],X[4],X[5], X[6], round(tt, digits = 2), round(pp, digits=2), nref, round(pm1[2], digits=2), round(pm1[1], digits=2), round(pm1[3], digits=2), round(summary(lm1)$r.squared, digits = 2), within, stringsAsFactors = FALSE)
	}

	sortlist <- split(sort, 1:nrow(sort))

	if(Sys.info()[['sysname']] == "Windows") {
		op <- system.time ( hera1 <- lapply(FUN = myfunante, X = sortlist) )
		print(op)
	}
	else {
		op <- system.time ( hera1 <- mclapply(FUN = myfunante, X = sortlist, mc.cores = cores, mc.preschedule = TRUE) )
		print(op)
	}

	hera1 = as.data.frame(data.table::rbindlist(hera1))
	colnames(hera1) <- c("id","antemortem_stature", "id","side","element","postmortem_measurement", "t-statistic", "p-value","sample_size", "lower_PI","point_estimate","upper_PI", "Rsquared", "Result")

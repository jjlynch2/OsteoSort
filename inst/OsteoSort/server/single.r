	...
	observeEvent(input$proc, {
		...
		if(testt == 'reg') {
			if(all(is.na(reghum[,4:length(reghum)])) && all(is.na(reguln[,4:length(reguln)]))) {removeModal(); return(NULL)}###stops crashing if empty
				...
				direc2 <- reg.multitest(sort = outputtemp1[[1]], ref = outputtemp1[[2]], splitn = outputtemp1[[3]], predlevel = input$alphalevels2, sessiontempdir = sessiontemp, oo = c(input$fileoutput3, input$fileoutput4))		

...
function ZTEST(m1, m2, RL, RR)
	...
	for x in 1:size(m1,1)
		for j in 1:size(m2,1)
			...
			Results[Results_Iterator,3] = 0 #not needed but left here so array columns match in R
			Results[Results_Iterator,4] = wZP #final P from norm
			Results[Results_Iterator,5] = 0 #since final is norm, return 0 for mean
			Results[Results_Iterator,6] = 1 #since final is norm, return 1 for sd
			Results[Results_Iterator,7] = size(refd,1) #reference sample size
			for s in 1:size(res,2)
				co = 1
				if res[s] == 1
					Results[Results_Iterator,7+s] = refd_mean[co]
					Results[Results_Iterator,7+(size(res,2)+s)] = refd_std[co]
					co += 1
				end
			end
			Results_Iterator += 1
